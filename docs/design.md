...
**输出：** PWM方波（3~5kHz, 占空比30~70%）→ 送入功率放大器

**接口：** 输出引脚接IRL540N栅极（通过前置放大）
...
**接口：** IRL540N漏极接电机正极，源极接GND，电机并联续流二极管
...
### 3.2 功率放大器 — IRL540N（逻辑电平MOSFET）

**选型理由：**
- **逻辑电平MOSFET**，Vgs=5V 即可完全导通（Rds(on)=0.077Ω @Vgs=5V）
- Vds=100V，Id=33A，远超电机需求（12V/0.5A），安全裕量大
- TO-220封装，散热好
- 与 NE555 的 5V 输出直接兼容，无需额外栅极驱动电路

**为什么不选 IRF540N：** IRF540N 的 Rds(on) 标定条件为 Vgs=10V，在 5V 栅极驱动下无法完全导通，会导致 MOSFET 过热和电机电压不足。

**备选方案：** IRF520（普通 MOSFET，同样存在 5V 驱动不足问题）、TIP122 达林顿管（压降大，功耗高）
...
| 条件 | R_freq | R_duty | R_total | f | D |
|------|--------|--------|---------|---|---|
| f=3kHz, D=30% | 18.7kΩ | 45.7kΩ | 65.4kΩ | 3.0kHz | 30% |
| f=5kHz, D=70% | 3.6kΩ | 6.2kΩ | 10.8kΩ | 5.0kHz | 70% |

**已确认参数：**
- R1 = 1kΩ (固定)
- R_freq = **20kΩ 电位器** (调频率)
- R_duty = **47kΩ 电位器** (调占空比)
- C = 10nF

**调节特性：** 两个电位器共同影响频率和占空比。增大 R_duty 降低频率并减小占空比；增大 R_freq 升高占空比但降低频率。实际使用时需配合调节。

### 模块2：功率放大器

**选定方案：** N沟道逻辑电平MOSFET (**IRL540N**)

**设计变更说明：** 原方案 IRF540N 的 Rds(on) 标定条件为 Vgs=10V，而本系统 NE555 输出仅 5V。在 Vgs=5V 下 IRF540N 无法完全导通，Rds(on) 显著增大，MOSFET 会严重发热。

**IRL540N 优势：**
- 逻辑电平 MOSFET，Vgs=5V 即可完全导通
- Rds(on) = 0.077Ω @ Vgs=5V，导通损耗极小
- Vds=100V，Id=33A，安全裕量充足
- TO-220 封装，与 IRF540N 引脚兼容

**续流二极管：** 1N4007 反向并联在电机两端，吸收关断时的反电动势。
...
**转速测量参数：**
- 假设电机转速范围：1000~5000 RPM（17~83 r/s）
- 码盘：1槽 → 脉冲频率 17~83Hz
- 闸门时间：1秒 → 计数值 = 转速 r/s，分辨率 1 r/s
- 2位数码管显示范围 00~99 r/s，覆盖预期转速范围
...
- 32.768kHz = 2^15 Hz
- CD4060 内置振荡器+14级分频 → Q14输出2Hz
- 74HC74 D触发器二分频 → 1Hz方波 (T=1s)

### 模块6：计数与显示

**推荐方案：**
- CD4518 (双BCD加法计数器) — 片内两个计数器级联实现2位十进制
- CD4511 (BCD-7段译码/锁存器) — 内置锁存(LE引脚)，无需外加锁存器
- 2位共阴极数码管
...
| 功率管 | IRL540N | 2 | 逻辑电平MOSFET，5V驱动 |
| 二极管 | 1N4148/1N4007 | 5 | 占空比调节+续流保护 |
| 电位器 | 20kΩ, 47kΩ | 各2 | 频率和占空比调节 |
...
- **电机参数：** 12V/≤0.5A，带测速码盘
- **仿真工具：** Multisim
- **功率放大方案：** 逻辑电平MOSFET (IRL540N)，5V栅极驱动
- **光电传感器：** 传感器模块（带PCB板）
